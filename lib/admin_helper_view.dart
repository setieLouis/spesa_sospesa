                  ...
                  child: CustomBtn(
                    ...
                    onPress: () async {
                      ...
                      if (result != null) {
                        ...
                        spese =
                            parseExcel(Excel.decodeBytes(file, update: true));
  ...
  List<Spesa> parseExcel(var excel) {
    ...
    for (var table in excel.tables.keys) {
      ...
      for (int i = 1; i < maxRow; i++) {
        ...
        family.boys = member.length > 1 ? int.parse(member[1]) : 0;
        family.baby = member.length > 2 ? int.parse(member[2]) : 0;
        ...
        for (int j = 4; j < maxCol; j++) {
          ...
          for (String el in cols) {
            spesa.addOne(isNumeric(el) ? " ${head[j]} $el" : "${el.trim()}");
          }
        }
